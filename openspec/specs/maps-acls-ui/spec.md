...
The account-facing SvelteKit management surface for maps and ACLs: the `/maps` list with create/edit/soft-delete, the slug-keyed map canvas and its `/settings` route with ACL attach/detach, the `/acls` list with create/rename/delete, the UUID-keyed `/acls/[id]` detail with member CRUD, and the entity-search-backed member picker (with client-side member-type/permission gating and the unavailable-vs-empty distinction). Covers the routes, load functions, form actions, and the `$lib/api.ts` client functions for these endpoints. Map drawing/canvas itself is out of scope (placeholder until built).
