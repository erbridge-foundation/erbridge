...
Shared SvelteKit-frontend primitives that span multiple capabilities. This capability is intentionally broad-named: the next shared frontend primitive (toasts, form-error rendering, loading states, etc.) can land here without spinning up a new capability. The initial scope is the destructive-action confirmation modal — a single shared component used by every state-mutating action whose effects are not symmetrically reversible from within the same view.
