...
Shared API contract for all `/api/*` routes: major-version URL prefix, JSON success/error envelopes, canonical error codes, RFC 3339 UTC timestamps, and a machine-readable OpenAPI 3.1 description of every route. Consumed by both backend handlers and frontend clients.
