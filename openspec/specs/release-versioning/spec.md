...
Defines how the application version and commit identifier are derived, propagated through the build, baked into the backend and frontend artifacts, and used by the CI publish pipeline. Git tags are the single source of truth for release versions; the manifest versions (`backend/Cargo.toml`, `frontend/package.json`) are non-authoritative sentinels. CI fetches full history, computes `APP_VERSION` and `GIT_COMMIT_SHA` on the host, and passes them into both image builds as build args; the backend exposes them via `GET /api/health` and the OpenAPI `info.version`, and the frontend exposes them via `PUBLIC_UI_VERSION` and `PUBLIC_GIT_COMMIT`. The publish workflow publishes only for `develop` (staging) and `v*` release tags (production), guards release-tag immutability with a two-layer protection (git ruleset + in-CI pre-flight existence check), and bootstraps the line at `v0.0.1` after the broken pre-fix images are cleaned out of GHCR.
