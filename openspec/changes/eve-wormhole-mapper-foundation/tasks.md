...
- [ ] 2c.11 **Re-verify §2c with the refactored handlers** (supersedes §2c.8). Boot the backend with `RUST_LOG=erbridge=debug,reqwest_tracing=info` so every outbound HTTP call logs one line. Then:
