...
- `GET /api/v1/me` — returns the caller's account plus their full character list, including `corporation_name` / `alliance_name` resolved from ESI public-info, `portrait_url` constructed from the EVE image server, and a derived `token_status` enum per character (see below). Raw token columns are NEVER included in the response.
...
**Per-character `token_status`.** Each element of `data.characters` SHALL carry a `token_status` field with the value `"active"` when the row's `esi_token_expires_at > now()`, and `"expired"` when `esi_token_expires_at` is in the past or NULL. The raw `esi_token_expires_at` timestamp SHALL NOT be returned (token metadata stays out of the wire). The field is a **string enum**, not a boolean, so future scope-set expansion (e.g. `"missing_scopes"` once a scope set is required that the existing token doesn't satisfy) can land without a breaking change. The characters-page UI renders a `--emerald` / `--red` dot driven entirely by this value.
...
- **No automatic token refresh** → Per the EVE SSO contract, the access token is short-lived (~20 minutes) and the refresh token carries the durable expiry (the value stored in `eve_character.esi_token_expires_at`). This change stores both encrypted but does not yet use the refresh token to mint a new access token on demand; the refresh-on-expiry flow is the next auth improvement, unblocked by tokens now being in Postgres. Until then, when the refresh token expires (`esi_token_expires_at <= now()`) the character's `token_status` flips to `"expired"` on `/api/v1/me` and the characters page surfaces a `re-auth` link that re-runs SSO (§12.5).
