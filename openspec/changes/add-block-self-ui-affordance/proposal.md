...
- No backend change. The backend guard remains the enforcement boundary; this is presentation-layer defence-in-depth.
...
- `server-administration`: the self-block rule gains a UI-affordance requirement — the block picker SHALL visibly mark the admin's own characters as non-selectable (no new behaviour on the request path; the existing `409 cannot_block_self` guard is unchanged).
...
- **Frontend only.**
  ...
  - `frontend/messages/{en,de,fr}.json` — new i18n keys for the badge label and title (all three locales, per repo rule).
  - `frontend/src/routes/admin/blocks/page.svelte.test.ts` — component coverage for the self badge.
