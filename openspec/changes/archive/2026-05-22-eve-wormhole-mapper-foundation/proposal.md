...
- Session management: Postgres-backed session store (`session` table keyed by session ID, holding `account_id` and a sliding 7-day `expires_at`) so sessions survive backend restarts; session cookie (`httpOnly`, `SameSite=Lax`) carrying an HS256 JWT of the session ID, refreshed on every authenticated request so the cookie's `exp` tracks the DB row — no token material exposed to the browser
...
- Account-management API under `/api/v1/`: `GET /me` (returns the caller's account and character list with corp/alliance names read directly from the row, portrait URLs, and a derived `token_status` enum per character — `"active"` when a refresh token is held, `"expired"` when not — without exposing any raw token material), `POST /characters/:id/set-main`, `DELETE /characters/:id` (with `cannot_remove_main` / `cannot_remove_last_character` guards), `DELETE /account` (soft-delete with session cookie clearing)
