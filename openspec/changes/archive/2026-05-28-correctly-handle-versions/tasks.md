...
- [x] 7.1 Cut and push the `v0.0.1` tag from `main`; confirm CI's fixed pipeline publishes `erbridge-api`/`erbridge-web` at `:v0.0.1` + `:latest` + `:sha-<short>` — done: `v0.0.1` (commit `b470da7`) published `:v0.0.1` + `:sha-b470da7` on both packages; a subsequent `v0.0.2` (commit `4e80953`) published `:v0.0.2` + `:latest` + `:sha-4e80953`, confirming `:latest` moves only on release tags and the tag pipeline works
- [x] 7.2 Verify the baseline artifacts: `erbridge-api` `/api/health` reports `version: 0.0.1` + real commit; `erbridge-web` `/about` shows UI version `0.0.1` + real UI commit — verified on the released images: `/api/health` reports the clean git-derived version + real commit (not `0.1.0`/`unknown`) and `/about` shows matching UI version + commit (confirmed on the `v0.0.2` release: UI `0.0.2 · 4e80953`, API `0.0.2 · 4e80953`)
- [x] 7.3 Delete the stale pre-fix GHCR image versions (`0.1.0` / `commit unknown`) — operator runs this with a `delete:packages`-scoped token (the session token has only `repo, workflow, read:org, gist`, so it cannot); confirm only correctly-versioned artifacts remain — done: the pre-fix `sha-05b3112` version (built by the broken pipeline) was deleted from both `erbridge-api` and `erbridge-web`. Remaining untagged GHCR rows are NOT dead artifacts — they are the per-architecture child manifests of the multi-arch `:v0.0.1` / `:v0.0.2` / `:develop` manifest lists (the package's "OS / Arch 2" indicator confirms multi-arch publishing; each tagged row has two same-minute untagged siblings = list + 2 arch children). They are load-bearing and SHALL NOT be deleted. Every remaining artifact in GHCR therefore traces back to a correctly-versioned release tag
