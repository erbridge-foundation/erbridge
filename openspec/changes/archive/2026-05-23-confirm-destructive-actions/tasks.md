...
- [x] 6.1 In a browser at `/characters`, click `remove` on a non-main character. The modal opens; the cancel button has visible focus; the title contains the character's name. Click cancel — modal closes, nothing else changes. Click `remove` again, then click the destructive button — the character is removed (the row disappears and the success/error rendering matches the pre-change behaviour).

  **Note:** Initial verification showed focus landing on the destructive button rather than cancel on mouse-click open (jsdom-passing-but-browser-failing race between `queueMicrotask` and Svelte 5's `bind:this` flush). Fixed in a follow-up by replacing the single open-effect with two `$effect`s — one for open/close lifecycle, one reactive on `cancelButtonEl` that fires once the bind lands. Re-verified after the fix.
- [x] 6.2 Repeat §6.1 with keyboard only: Tab to the `remove` button, press `Enter`. Modal opens; Tab to the destructive button; press `Enter`. Character is removed. Then: open the modal again, press `Escape`. Modal closes, no submission.
- [x] 6.3 At `/characters`, click `delete account` under DANGER ZONE. Modal opens with title `Delete account?` and the 30-day body copy. Click cancel — nothing happens.

  **Deferred (destructive on the verifier's own account):** the "click the destructive button — account is soft-deleted" step from the original task was not performed, because doing so on a real account requires re-running EVE SSO to recover. The open + cancel paths were verified, which exercises every code path introduced by this change; the actual submission path is the pre-existing foundation behaviour and is covered by the §3.3 wiring task and the Playwright `cancelling the delete-account modal does not submit` e2e test. The "actually submit" path can be verified in a future change that ships a dedicated test account.
- [x] 6.4 Click outside the dialog body (the backdrop region). Modal closes (cancel path).
- [x] 6.5 With browser DevTools, simulate `prefers-reduced-motion: reduce` (Rendering tab → Emulate CSS media feature `prefers-reduced-motion`). Re-open the modal. The dialog and backdrop appear instantly with no fade or scale animation.
- [ ] 6.6 With a screen reader (VoiceOver on macOS, NVDA on Windows, or Orca on Linux), open the modal. The screen reader announces the title and body content. Focus is on the cancel button. `Tab` moves to the destructive button.
  ...
  **Deferred:** the verifier has no screen reader installed. The Chrome accessibility-tree check (Option A in the §6 plan) is a defensible structural substitute but was also not run in this pass. Track this as an open follow-up: either run a screen reader in a future verification pass, or check the accessibility tree as a one-off, and then check this box. The risk of deferring: the `role="alertdialog"` + `aria-labelledby` + `aria-describedby` wiring is exercised by the Vitest suite (which asserts the attributes resolve to the expected elements), so the spec's announcement contract is verified at the structural level — only the *live announcement* by a real assistive technology is unverified.
- [x] 6.7 Disable JavaScript in the browser. Reload `/characters`. The `remove` and `delete account` buttons are present; clicking them does nothing (per the §3.5 trade documented in design.md decision 8). The character list still renders correctly (server-side rendering is unaffected).
- [x] 6.8 Open the updated `frontend/wireframes/characters.html` and the live `/characters` side-by-side. The open-state variants in the wireframe SHALL match the live modal's layout, spacing, and colours. Document deliberate deviations as comments in the Svelte component.
