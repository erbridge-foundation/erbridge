...
- [x] 1.1 Add `@xyflow/svelte` to `frontend` (pnpm). Confirm it's the ONLY new dependency.
- [x] 1.2 Add map design tokens to `src/app.css`: class colours `--c1..--c6`, security
...
- [x] 2.1 `src/lib/map/types.ts`: define the position-less combined-graph contract
  (`System`, `Connection` with `origin`/`mass`/`eol`/`wh_type`/`sig_*`, `Tab` with a root
  SET, `CombinedGraph = { systems, connections }`, `LocalState`).
- [x] 2.2 `src/lib/fixtures/map-canvas.ts`: a static snapshot exercising every render state
...
- [x] 2.3 Provide a *second* server-state snapshot (or a delta) for the simulated update so
  the reconcile path (new node, departed node, ghost→confirmed) can be demonstrated.

## 3. Layout (Fork 2 — hand-rolled BFS, no lib)

- [x] 3.1 `src/lib/map/layout.ts`: BFS rank from `tab.roots` (min hop across the root set);
...
- [x] 3.2 Unit-test layout: determinism (same input → same output), multi-root min-hop rank,
  the three directions, ghost parking.

## 4. Placement + reconcile (Fork 1)

- [x] 4.1 `src/lib/map/placement.ts`: thin seam over `localStorage[mapId]` — `load(mapId)`,
  `save(mapId, positions)` (debounced), `clearTab(mapId, tab)`. Keep it swappable (the real
  backend is a later Track-2 decision).
- [x] 4.2 `src/lib/map/reconcile.ts`: `combined = serverState ∪ localState`;
...
- [x] 4.3 Unit-test reconcile: survive-restart (saved beats seed), new-takes-seed,
...
- [x] 5.1 `src/lib/components/MapCanvas.svelte`: mounts svelte-flow against the combined
...
- [x] 5.2 `src/lib/components/map/SystemNode.svelte`: class/security/static badges as TEXT +
  colour decoration; root indicator; ghost styling for local-only systems.
- [x] 5.3 `src/lib/components/map/ConnectionEdge.svelte`: wh-type label; mass as a TEXT cue
  (fresh/half/critical) + colour; EoL `⚠` glyph + pulse (decoration only, drops under
  reduced-motion with no info loss).
- [x] 5.4 Component-test the encoding rules: mass readable without colour; EoL readable
  without motion; class/sec/statics render as text.

## 6. Sandbox route

- [x] 6.1 `src/routes/maps/_proto/+page.svelte`: mount `MapCanvas` with the fixture. NO
  `+page.server.ts`, NO loader, NO auth — pure static. Real `/maps/[slug]` stays untouched.
- [x] 6.2 e2e (Playwright): load `/maps/_proto`; nodes/edges render; drag a node and reload →
...
- [x] 7.1 Update `openspec/AGENTS.md`: add `/maps/_proto` to the route tree and `MapCanvas`
  (+ `$lib/map/*`, custom node/edge components) to the component tree; note the new map tokens.
- [x] 7.2 Root docs: no config/deploy/setup facts change (sandbox route, no env vars) — confirm
...
- [x] 8.1 `pnpm test` — Vitest unit/component tests
- [x] 8.2 `pnpm run check` — svelte-check (type-check + paraglide compile)
- [x] 8.3 `pnpm run test:e2e` — Playwright e2e
