...
- New `.github/workflows/backend.yml` (or equivalent runner — final venue chosen in design.md) running `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, and `./backend/scripts/test.sh` on every push and PR that touches `backend/**`.
