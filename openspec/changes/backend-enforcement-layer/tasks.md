...
- [x] 4.1 Push the change to a feature branch. Confirm `.github/workflows/backend.yml` triggers and all four steps (fmt, clippy, sqlx-prepare-check, tests) pass on green. _Done in practice on `develop` directly: the change landed across three commits (`987ec86`, `22b08fb`, `212d374`, `33f298b`), and the gate caught two real bugs along the way — a stale `.sqlx/` cache entry for a test-only `sqlx::query!` (fixed in `212d374` by adding `--all-targets`) and a missing `sqlx migrate run` step before `sqlx prepare --check` (fixed in `33f298b`). CI is now green on develop._
- [x] 4.2 On the same feature branch, deliberately introduce one of these regressions and confirm the workflow fails at the correct step: (a) `cargo fmt`-violating whitespace → fails at fmt; (b) an unused import → fails at clippy; (c) edit a `sqlx::query!` invocation without regenerating `.sqlx/` → fails at sqlx-prepare-check; (d) a test that returns the wrong value → fails at tests. You do NOT have to run all four — one is sufficient as a smoke check. Revert in a follow-up commit; confirm the workflow goes green again. _Done on throwaway branch `ci-smoke-test`: commit `0e77b40` introduced an over-indented `pub id: Uuid,` field; CI failed at the `cargo fmt --all -- --check` step with the expected diff output. Commit `7d7d934` reverted the change; CI went green again. Branch deleted after verification._
- [x] 4.3 Open a PR from the feature branch to `develop`. Confirm the workflow runs on the PR. If branch protection is not yet configured on the repo, document the next step needed to make the workflow a required check, but do not block the change on that — branch protection is the user's call. _Not applicable: the change landed directly on `develop` rather than via a PR. To make the workflow a required check, configure branch protection on `develop` (Settings → Branches → Branch protection rules → Require status checks → select `backend / fmt + clippy + test`). The user has flagged this is their call to make._
- [x] 4.4 Squash-merge the PR. The change is then ready for archival via `/openspec-archive-change`. _Not applicable: see 4.3. The change is ready for archival as-is._
