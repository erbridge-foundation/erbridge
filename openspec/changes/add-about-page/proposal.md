...
- `api-health`: defines `GET /api/health` — its response shape, public access, component-aggregation rule, and the requirement that it lives outside the `AuthenticatedAccount` middleware tree alongside `/api/openapi.json` and `/api/docs`.
