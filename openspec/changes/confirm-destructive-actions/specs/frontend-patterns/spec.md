...
This capability defines shared SvelteKit-frontend primitives that span multiple capabilities. It is intentionally broad-named: the next shared frontend primitive (toasts, form-error rendering, loading states, etc.) can land here without spinning up a new capability. The initial scope is the destructive-action confirmation modal.
