...
- [x] 6.1 Build the backend image with `APP_VERSION`/`GIT_COMMIT_SHA` and confirm `GET /api/health` returns the real version + commit (not `0.1.0`/`unknown`) and OpenAPI `info.version` matches — verified the build-time baking deterministically: a compile with `APP_VERSION=1.2.3 GIT_COMMIT_SHA=testc0m` yields `env!("CARGO_PKG_VERSION")==1.2.3` / `env!("GIT_COMMIT_SHA")==testc0m`, and with no `APP_VERSION` falls back to the `0.0.0` manifest sentinel. `health.rs` (version+commit) and `openapi.rs` (`info.version`) read the identical `env!` macros, so the HTTP/OpenAPI surfaces are a passthrough of these constants and necessarily match (the live HTTP probe additionally needs Postgres + ESI to boot `main()` — see 7.x baseline release for the end-to-end published-image check)
- [x] 6.2 Build the frontend image and confirm `/about` shows the git version + UI commit — built `erbridge-web` with `APP_VERSION=1.2.3 GIT_COMMIT_SHA=testc0m`, ran the container, and confirmed `GET /about` SSRs both `1.2.3` and `testc0m` into the HTML (and the values are inlined in the about-page client chunk)
